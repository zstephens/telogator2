...
def main(raw_args=None):
    ...
    if OUT_READS[-2:].lower() == 'fq' or OUT_READS[-5:].lower() == 'fq.gz' or OUT_READS[-5:].lower() == 'fastq' or OUT_READS[-8:].lower() == 'fastq.gz':
        OUTPUT_IS_FASTQ = True
    INPUT_IS_FASTQ = False
    if IN_READS[-2:].lower() == 'fq' or IN_READS[-5:].lower() == 'fq.gz' or IN_READS[-5:].lower() == 'fastq' or IN_READS[-8:].lower() == 'fastq.gz':
        INPUT_IS_FASTQ = True
    ...
    while True:
        ...
        if any(got_hits):
            ...
            if READTYPE == 'CCS':
                ...
                #
                if len(readname_splt) >= 2:
                    if readname_splt[1][:3] == 'np=':
                        my_np = int(readname_splt[1][3:])
                        if my_np < MIN_NP:
                            read_passes_filters = False
                    elif readname_splt[1][:3] == 'np:':
                        my_np = int(readname_splt[1].split(':')[-1])
                        if my_np < MIN_NP:
                            read_passes_filters = False
                #
                if read_passes_filters and len(readname_splt) >= 3:
                    if readname_splt[2][:3] == 'rq=':
                        my_rq = float(readname_splt[2][3:])
                        if my_rq < MIN_RQ:
                            read_passes_filters = False
                    elif readname_splt[2][:3] == 'rq:':
                        my_rq = float(readname_splt[2].split(':')[-1])
                        if my_rq < MIN_RQ:
                            read_passes_filters = False
                #
